...
let p={x:W/2,y:H/2,r:14,hp:100,max:100,speed:250,damage:18,rate:.48,bulletSpeed:600,multi:1,pierce:0,magnet:90,orbit:0};
...
function reset(){
  ...
  p={x:W/2,y:H/2,r:14,hp:100,max:100,speed:250,damage:18,rate:.48,bulletSpeed:600,multi:1,pierce:0,magnet:90,orbit:0};
...
const upgrades=[
  ['Overcharge','+35% projectile damage',()=>p.damage*=1.35],
  ['Rapid Core','Fire 18% faster',()=>p.rate*=.82],
  ['Twin Shot','+1 projectile',()=>p.multi=Math.min(5,p.multi+1)],
  ['Phase Pierce','Projectiles pierce +1 enemy',()=>p.pierce++],
  ['Thrusters','Move 15% faster',()=>p.speed*=1.15],
  ['Vital Matrix','+30 max HP and heal',()=>{p.max+=30;p.hp=Math.min(p.max,p.hp+30)}],
  ['Magnet Field','Pickup range +45%',()=>p.magnet*=1.45],
  ['Orbit Blade','Add a rotating energy blade',()=>p.orbit=Math.min(6,p.orbit+1)],
  ['Velocity','Projectile speed +25%',()=>p.bulletSpeed*=1.25],
  ['Repair','Restore 45 HP',()=>p.hp=Math.min(p.max,p.hp+45)]
];

function levelUp(){
  paused=true;ui.level.classList.remove('hide');ui.choices.innerHTML='';
  [...upgrades].sort(()=>Math.random()-.5).slice(0,3).forEach(u=>{
    let b=document.createElement('button');b.className='choice';b.innerHTML='<b>'+u[0]+'</b>'+u[1];
    b.onclick=()=>{u[2]();ui.level.classList.add('hide');paused=false};ui.choices.appendChild(b)
  })
}
...
function fire(){
  ...
  let e=enemies.reduce((a,b)=>Math.hypot(a.x-p.x,a.y-p.y)<Math.hypot(b.x-p.x,b.y-p.y)?a:b);
  let a=Math.atan2(e.y-p.y,e.x-p.x);
  for(let i=0;i<p.multi;i++){
    let off=(i-(p.multi-1)/2)*.13,aa=a+off;
    bullets.push({x:p.x,y:p.y,vx:Math.cos(aa)*p.bulletSpeed,vy:Math.sin(aa)*p.bulletSpeed,r:5,life:1.8,pierce:p.pierce})
  }
}
...
function update(dt){
  ...
  if(p.orbit){
    for(let j=0;j<p.orbit;j++){
      let a=t*2.4+j*6.28/p.orbit,ox=p.x+Math.cos(a)*58,oy=p.y+Math.sin(a)*58;
      enemies.forEach(e=>{
        if(Math.hypot(e.x-ox,e.y-oy)<e.r+8&&e.hit<=0){e.hp-=p.damage*.65;e.hit=.16;burst(e.x,e.y,3)}
      })
    }
  }
  ...
  bullets.forEach(b=>enemies.forEach(e=>{
    if(b.life>0&&Math.hypot(b.x-e.x,b.y-e.y)<b.r+e.r){
      e.hp-=p.damage;b.life=b.pierce>0?(b.pierce--,b.life):-1;burst(b.x,b.y,3)
    }
  }));
...
function draw(){
  ...
  bullets.forEach(b=>{x.fillStyle='#fff';x.shadowBlur=15;x.shadowColor='#4beaff';x.beginPath();x.arc(b.x,b.y,b.r,0,7);x.fill()});
  x.shadowBlur=0;
  ...
  if(p.orbit)for(let j=0;j<p.orbit;j++){
    let a=t*2.4+j*6.28/p.orbit;x.save();x.translate(p.x+Math.cos(a)*58,p.y+Math.sin(a)*58);x.rotate(a);
    x.fillStyle='#d861ff';x.shadowBlur=18;x.shadowColor='#d861ff';x.fillRect(-9,-4,18,8);x.restore()
  }
  x.shadowBlur=0;
